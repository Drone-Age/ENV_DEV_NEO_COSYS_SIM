[CmdletBinding()]
param(
    [Parameter(Mandatory)][ValidateSet('doctor', 'setup', 'build', 'run', 'test', 'camera-test', 'ros-test', 'vins-test', 'wind-test', 'iros-test', 'ivins', 'stop', 'logs', 'env', 'capabilities')][string]$Command,
...
function Invoke-IvinsWsl([string]$WslCommand, [string]$Operation) {
    # PowerShell here-strings use CRLF in this repository. Bash treats the
    # trailing CR as part of tokens such as `set -e`, file paths and package
    # names, so normalize only the command payload before crossing into WSL.
    $normalizedCommand = $WslCommand.Replace("`r`n", "`n").Replace("`r", "`n")
    $result = Invoke-Wsl -Command $normalizedCommand -AllowFailure
...
function Assert-IvinsInstalledRuntime {
    # Keep the payload on one line: Windows PowerShell's legacy native
    # argument marshalling is not reliable for a multi-line `bash -lc` value.
    $checks = @(
        ...
        'dpkg-query -W ivins ivins-installer-agent iros2j-ros-core imavros vins-mono-ros2 vio-stack'
        'echo IVINS_DOCTOR_PASS'
    )
    $command = $checks -join ' && '
...
function Start-VinsStack([string]$RunDirectory, [object]$Settings, [switch]$EnableWind) {
    ...
    $command = "bash '$launcher' '$repoWsl' '$runWsl' '$($Settings.Network.WindowsIp)' '$($script:Config.ports.cosys_rpc_tcp)' '$($script:Config.ports.mavlink_tcp)' '$($script:Config.ports.wind_mavlink_tcp)' '$domainId' '900' '$enableWindValue' '$IvinsRuntime'"
    $result = Invoke-Wsl -Command $command -AllowFailure
