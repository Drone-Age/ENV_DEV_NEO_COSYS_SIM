...
- [done] Blocks executes `TAKEOFF -> 15 m square -> LAND -> DISARM` without Mission Planner. The post-backend-fix regression `2026-08-25_121753_test_4f11822d` reached 5.71 m, completed every waypoint and landed/disarmed.
...
- [done] Provide live-SITL 640 x 480 and 1280 x 720 raw-RGB evidence after removing the ArduCopter backend's blocking game-thread UDP loop. Acceptance remains >=20 FPS and >=10 FPS respectively. The current Blocks bundles pass at 26.77 FPS / 24.0 worst-window (`2026-08-25_123234_camera-blocks-640x480_a0e94400`) and 24.03 FPS / 18.5 worst-window (`2026-08-25_123348_camera-blocks-1280x720_8f7d3968`), with zero duplicate timestamps and valid image content. The same 1280 x 720 run measured PNG at only 6.89 FPS, so realtime transport remains raw RGB.
- [in progress] The 640 x 480 ROS path passes at 21.76 FPS / 19.5 worst-window with batched IMU (`2026-08-25_075418_test_91c0c67e`). The 1280 x 720 ROS/VINS end-to-end transport still requires the same window gate before qualification.
...
- [in progress] Preserve the production iHUB/UART gimbal contract. The pinned iHUB compatibility branch now uses steady transport clocks, tolerates isolated Cosys RPC timeouts, rate-limits camera-pose traffic, and passes 21 unit tests. Runtime evidence `2026-08-25_122627_test_c0e506dc` demonstrated fault-free UART, the complete 0-90 degree sweep and 1.57 rad/s moving CameraImu after calibration. Automatic VINS admission remains open because Blocks loses features at the nadir angle and the initializer still needs a flight/texture-qualified excitation profile.
