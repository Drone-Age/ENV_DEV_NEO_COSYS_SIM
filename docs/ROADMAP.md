...
- [in progress] After the successful flight baseline, harden camera cadence: report p50/p95/p99/max inter-frame gaps and the worst full two-second window; require that neither supported resolution falls below 10 unique FPS in any full two-second window. Windows-native 1280 x 720 passes at 20.56 FPS / 19.5 FPS worst window with 0 duplicate timestamps. The repaired 640 x 480 ROS path now passes repeatedly over WSL2 NAT at 18.16-21.20 wall FPS with 15-19 FPS worst windows, and sustained 23.31 bridge FPS through the complete Blocks flight (`2026-08-25_064520_test_1220bb93`). The 1280 x 720 ROS/VINS transport still requires the same window gate before qualification.
