...
`env build-map sim2-rural` stages the LFS-pinned binary map, compiles the small environment-owned UE editor commandlet, applies the always-loaded physics metadata contract, replaces legacy primitive trees with the accepted rural asset, then reloads and verifies the georeference, transform, imagery material, 256 Landscape proxies and all 1024 render/collision components. Ordinary deployment does not re-import the 4033 x 4033 Landscape; raw GIS regeneration is an explicit authoring operation because UE 5.8.1 can race while repartitioning a newly imported Landscape.

The packaged map already contains accepted rural assets. Maintainers can reproduce the narrowly scoped UE 5.8 ArchVis Example tree, four Poly Haven CC0 ground-cover meshes, and Quaternius CC0 mature corn/wheat with `.\dev.ps1 env import-assets sim2-rural`. The command validates every pinned source size and SHA-256, builds the required Unreal materials, and records closed dependency sets; it does not rebuild third-party binary packages.

`sim2-rural` is currently marked `preview`. Normal `build`, `run` and flight tests reject it; an engineering validation must opt in with `-Preview`, which is recorded in `summary.json` and cannot promote readiness. `env doctor sim2-rural -Preview` validates package integrity, while `env build-map` is the structural acceptance command. The environment doctor also validates every declared asset receipt: its status must match the manifest, both receipt and source-lock paths must remain inside the environment package, and the source lock must exist. An absolute workstation path or `..` escape is a deployment failure, not a warning. For field crops it additionally resolves the declared provenance record and accepts a baseline species only when the same mesh id is both pinned in the source receipt and present in the closed Unreal validation report. This deliberately keeps sunflower outside the baseline until a licensed, reproducible package is acquired. The first clean map build can spend several minutes compiling UE shaders; wait while UnrealEditor-Cmd/ShaderCompileWorker remain active and require the final reload verifier PASS.

The current preview supports a full 15 m square flight directly on the DEM. The temporary pad is removed. The fail-closed runtime gate checks 1284 Landscape traces: all 256 proxy centres, both sides of 480 seams and 68 boundary points. Crop v4 retains 11,025 corn and 118,335 wheat instances in two real OSM polygons and adds one bounded 8,694-corn trial parcel 300 m east of origin, for 19,719 corn instances in total. The no-Mission-Planner flight bundle `2026-08-28_212236_test_35bd1e88` reached every waypoint, landed and disarmed with a 5.77 m maximum altitude. Its preceding camera bundles `2026-08-28_211741_camera-sim2-rural-640x480_3916fc2b` and `2026-08-28_211945_camera-sim2-rural-1280x720_b07f07d7` measured 28.85 and 28.78 raw unique FPS, with worst two-second windows of 28.0 and 27.5 FPS, zero duplicate timestamps and non-uniform pixels. The independently published ESA WorldCover mask classifies 88.830584% of the 4 x 4 km core as real cropland; a clean authoring machine reproduces it with `fetch-landcover`, `build-landcover`, `verify-landcover` and `publish-landcover`. `env build-map sim2-rural` imports it as a 4096 linear mask and applies the low-cost `worldcover-fields-v1` Landscape layer over Sentinel-2. Keep `readiness: preview`: near-camera crop geometry remains deliberately limited, crop species are synthetic, and the scene still needs more vegetation variants and improved horizon composition.

`cesium-global` remains a `scaffold` visual package. `dev.ps1 env build-map cesium-global` downloads the official prebuilt Cesium for Unreal 2.28.0 package for UE 5.8, verifies its pinned SHA-256 and engine BuildId, installs it under the discovered UE `Engine\Plugins\Marketplace`, and caches an installation receipt. It does not rebuild Cesium source. The generated map contains no token; set `INDRA_CESIUM_ION_TOKEN` only in the local process environment for a deliberate visual streaming run. Without a token, both tilesets remain dormant, no ion request is made, and a 40 m streaming safety pad prevents falling into the void. Do not promote this package beyond `scaffold` until credentialed terrain/building streaming and visual observation are recorded.
...
- `third_party/Cosys-AirSim` at `7acd746d849ab6b3dcc77d493740151d0b6f6800`, fork branch `indra-ue5.8`, based on release `5.8-v3.4.1`.
...
5. Pinned iMAVROS, VINS-NEO, iHUB, VINS initializer and vision bridge overlay under WSL ext4 at `~/.local/share/indra-cosys/vins-overlay-jazzy`. Build/install/log trees intentionally stay off DrvFS; this avoids multi-minute Python import and generated-interface install stalls while the repository remains the authoritative source.
...
4. Run `dev.ps1 build`, then the smallest acceptance sequence: `test`, `ros-test`, `vins-test`, `wind-test`. Automated commands must not launch Mission Planner.
